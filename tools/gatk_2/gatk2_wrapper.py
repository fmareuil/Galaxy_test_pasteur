...
GALAXY_EXT_TO_GATK_EXT = { 'fasta':'fa', 'gatk_interval':'intervals', 'bam_index':'bam.bai', 'gatk_dbsnp':'dbSNP', 'picard_interval_list':'interval_list' } #items not listed here will use the galaxy extension as-is
...
def __main__():
    ...
    if options.datasets:
        for ( dataset_arg, filename, galaxy_ext, prefix ) in options.datasets:
            ...
            if dataset_arg == "-R":
               dict_filename = os.path.join( tmp_dir, prefix+".dict" )
               cmd1 = 'CreateSequenceDictionary R=%s O=%s' % ( gatk_filename, dict_filename )
            if galaxy_ext == "bam":
    ...
    if cmd1:
        cmds.insert(0, cmd1)
    for c in cmds:
        proc = subprocess.Popen( args=c, stdout=stdout, stderr=stderr, shell=True, cwd=tmp_dir )
        return_code = proc.wait()
        if return_code:
