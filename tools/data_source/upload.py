...
import tarfile #add new  compressed type
...
from galaxy.datatypes.data import * #add new  compressed type
...
def parse_outputs( args ):
    ...
    return rval
def add_file( dataset, registry, json_file, output_path, uncompress_flag ):
    data_type = None
    line_count = None
    converted_path = None
    stdout = None
    ...
    # add new  compressed type
    if uncompress_flag == "no":
        ...
        if is_gzipped and is_gzvalid:
            if tarfile.is_tarfile( dataset.path ):
                data_type = "tar.gz"
                ext = "tar.gz"
        ...
        is_bzipped, is_bzvalid = check_bz2( dataset.path )
        if is_bzipped and is_bzvalid:
            if tarfile.is_tarfile( dataset.path ):
                data_type = "tar.bz2"
                ext = "tar.bz2"
            if re.search("fastq", os.path.basename(dataset.name)):
...
def __main__():
    ...
    for line in open( sys.argv[3], 'r' ):
        ...
        if dataset.type == 'composite':
            ...
            add_file( dataset, registry, json_file, output_path, uncompress_flag )
