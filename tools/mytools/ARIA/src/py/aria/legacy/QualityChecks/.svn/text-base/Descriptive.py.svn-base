class Descriptive:
    ...
    author: Lutz Ehrlich, EMBL Heidelberg
    """
    ...
    def ascii(self):
        if not self.getMean():
            ...
            meanOut = self.getMean()
        if not self.getStdDev():
            stdDevOut = 0.0
        else:
            stdDevOut =self.getStdDev()
        if not self.getMin():
            minOut = 0.0
        else:
            minOut = self.getMin()
        ...
        return "%8.3g  %8.3g  %8.3g  %8.3g" %(meanOut,stdDevOut,\
                                              minOut,maxOut)
    ...
    def latex(self):
        ...
        if not self.getMax():
            ...
            maxOut = self.getMax()
        return "%8.3g &  %8.3g &  %8.3g &  %8.3g " %(meanOut, 
                                                     stdDevOut,\
                                                     minOut,\
                                                     maxOut) + r"\\" + '\n'
...
if __name__ == '__main__':
    print 'testing...'
    someNumbers = [0.71, 0.75, 0.69, 0.70, 0.74]
    someNumbers = [0.112, 0.123]
    print 'input data:'
    print someNumbers
    desc = Descriptive()
    desc.addData(someNumbers)
#    print desc.getMean()
#    print desc.getSum()
#    print desc.getVariance()
    print desc
