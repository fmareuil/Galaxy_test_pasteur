...
C get the class name:
      CALL NEXTA4('class-name=',CLASS)
      CALL NEXTI('NOE exponent=',ITEMP)
      DO I=1,NOECCN
      CALL EQSTWC(NOECNM(I),4,CLASS,4,1,1,MATCH)
      IF (MATCH) NOEEXP(I)=ITEMP
      END DO
C====================================================================
...
C get the class name:
      ...
      END DO
C====================================================================
      ELSE IF (WD(1:4).EQ.'ASQC') THEN
C
C get the chi2 values in NOERM2:
      CALL NOEPRI(1000.0D0, 0,.FALSE.,0)
C
C get the class name:
      CALL NEXTA4('class-name=',CLASS)
      DO I=1,NOECCN
...
C assign the force constant automatically as Kboltz*T*N/chi2
C we use T=300 since this is the value used for the CSDX force field
      NOECNS(I)=KBOLTZ*NOETEM*FLOAT(NOENU2(I))/(TWO*NOERM2(I))
      WRITE(6,'(A,A,A,F15.5)')' NOE: weight for class ',NOECNM(I),':', 
     &  NOECNS(I)
      END IF
      END IF
      END DO
...
C====================================================================
      ...
      CALL NOEPRI(THRESH,0,.TRUE.,0)
      IF (NOEICV.GT.0) CALL NOEPRI(THRESH,1,.TRUE.,0)
      END IF
C====================================================================
      ELSE IF (WD(1:3).EQ.'RMS') THEN
      THRESH=100.0D0
      CALL NEXTA4('class-name=',CLASS)
      ICL1=-1
      DO I=1,NOECCN
      ...
      IF (MATCH) ICL1=I
      END DO
      CALL NOEPRI(THRESH,0,.FALSE.,ICL1)
      IF (NOEICV.GT.0) CALL NOEPRI(THRESH,1,.FALSE.,ICL1)
C====================================================================
...
C====================================================================
     ...
     &           NOECVO,NORSMX,NATOM,ISLCT,NISLCT,
     &           JSLCT,NJSLCT,X,Y,Z)
C
...
C I/O
      ...
      INTEGER NATOM, ISLCT(*), NISLCT, JSLCT(*), NJSLCT, NORSMX
      DOUBLE PRECISION X(*), Y(*), Z(*)
C local
      INTEGER I, II, JJ, J, TISLCT, TJSLCT, PEAKID, OLDORN
      LOGICAL SUCCES,OK
C begin
      NPEAKI=NPEAKI+1
      PEAKID=0
      ...
      OLDORN=NOEORN
C
      WRITE (6,'(A,I5)') ' reading restraint ', NPEAKI
...
C initialize frequencies
      ...
      NOEHP1(NOENUM)=-999.0D0
      NOEHP2(NOENUM)=-999.0D0
C initialize volume
      NOEVOL(NOENUM)=ZERO
      NOECVO(NOENUM)=ZERO
      NOECDI(NOENUM)=ZERO
...
C
      ...
      WRITE(6,'(A)')' %NOESET-ERR: error in selection - no atoms spec.'
      ELSE
      SUCCES=.TRUE.
      NOEORN=NOEORN+1
C
...
C default parameters are parsed, check for additional keywords
      ...
      CALL NEXTI('SPECtrum=',NOESPC(NOENUM))
      ...
      CALL NEXTF('HPM(F1)=',NOEHP1(NOENUM))
      ELSE IF (WD(1:4).EQ.'HPM2') THEN
      CALL NEXTF('HPM(F2)=',NOEHP2(NOENUM))
...
C
C update maximum number of protons in each group
C      TISLCT=TISLCT+NISLCT
C      TJSLCT=TJSLCT+NJSLCT
C      NOESMX=MAX(NOESMX,2*TISLCT)
C      NOESMX=MAX(NOESMX,2*TJSLCT)
C
      CALL MAKIND(ISLCT,NATOM,NISLCT)
      CALL MAKIND(JSLCT,NATOM,NJSLCT)
C
...
C
      IF (.NOT.SUCCES) THEN
      WRITE (6,'(A,I5,4F10.3)') ' %NOE-ERR: problem at ',
     &    PEAKID, NOEPP1(NOENUM),NOEPP2(NOENUM),
     &    NOEHP1(NOENUM), NOEHP2(NOENUM)
      NOENUM=NOENUM-1
C we do not need to reduce NOEORN since it is only advanced if SUCCES
      ELSE
      WRITE(6,'(A,I5)') ' restraint successfully read: ', PEAKID
      NOEPID(NOENUM)=PEAKID
      NOEORR(NOENUM+1)=NOEORN
      END IF
C
      OLDORN=NOEORN-OLDORN+1
      NORSMX=MAX(NORSMX,OLDORN)
C
      RETURN
      END
C======================================================================
...
C====================================================================
     ...
     &           NOECND,NOERAV,NOERRV,NOESTP,NOEDIS,NOELOW,NOEHIG,
     &           NOEWGH,NOECV,NBOUNDS,BOUNDS,SCRAMBL,BSYMM,BUNSYM,
     &           UNIQUE,BYPASS,PUNIT)
C
...
CMNTODO: implement noeorr array, for simplest case
     ...
     &           NOELOW,NOEHIG,NOEWGH,NOECDI,NOECV)
C
...
C I/O
      ...
      INTEGER NOEJLS(*), NOECND(*)
      DOUBLE PRECISION NOERAV(*), NOERRV(*), NOEDIS(*)
      DOUBLE PRECISION NOELOW(*), NOEHIG(*), NOEWGH(*), NOECDI(*)
...
C update maximum number of protons in each group
      ...
      DDX=ALLHP(IREAL8(NORSMX*NOESMX*NOESMX))
      DDY=ALLHP(IREAL8(NORSMX*NOESMX*NOESMX))
      DDZ=ALLHP(IREAL8(NORSMX*NOESMX*NOESMX))
      CALL ENOE2(EN,'ANAL',1,HEAP(DDX),HEAP(DDY),HEAP(DDZ),
     &     NOEORR,NOEIPR,NOEILS,
     &     NOEJPR,NOEJLS,NOECND,
     &     NOERAV,NOERRV,NOESTP,NOEDIS,
     &     NOELOW,NOEHIG,NOEWGH,NOECDI,NOECV)
      CALL FREHP(DDX,IREAL8(NORSMX*NOESMX*NOESMX))
      CALL FREHP(DDY,IREAL8(NORSMX*NOESMX*NOESMX))
      CALL FREHP(DDZ,IREAL8(NORSMX*NOESMX*NOESMX))
...
C
      ...
      END
C====================================================================
      SUBROUTINE NOEPRI(THRESH,ITEST,QPRINT,ICL)
C
...
C begin
     ...
     $           HEAP(HPNSPC),THRESH,QPRINT,ITEST,ICL)
...
C====================================================================
     ...
     &     NOESTP,NOECV,NOEPID,NOESPC,THRESH,QPRINT,ITEST,ICL)
      IMPLICIT NONE
C     I/O
      INCLUDE 'cns.inc'
      INCLUDE 'noe.inc'
...
C     
C     local
      DOUBLE PRECISION EN, NOERMS
C     noerm2 and noenu2 now in noe.inc
C     DOUBLE PRECISION NOERM2(NOECMX)
      DOUBLE PRECISION NOEVSM, NOEVS2(NOECMX)
      DOUBLE COMPLEX DBCOMP
      DOUBLE PRECISION DBPREC
C     INTEGER NOENU2(NOECMX)
      INTEGER NOEVIT
      INTEGER N, K, I, J, CLASS, NOENUML, NORR, FIRSTCLASS, LASTCLASS
      CHARACTER*6 SSAVE
      CHARACTER*11 SSPOT
      LOGICAL QHEAD
C     begin
C
      ...
      NOENUML=0
C
C     if a class is specified (ICL>0) use it, else loop over all
C     classes. if class not known (icl<0) skip calculation
      IF (ICL.GT.0) THEN
         FIRSTCLASS=ICL
         LASTCLASS=ICL
      ELSE
      ...
      END IF
C
      IF (NOENUM.GT.0.AND.ICL.GE.0) THEN
C     
         IF (QPRINT) THEN
            IF (NOEICV.GT.0) THEN
               IF (ITEST.EQ.0) THEN
     ...
     &                 ')  $$$$$$$$$$$$$$$$$$$$$$'
         ...
         END IF
C     
         DO CLASS=FIRSTCLASS,LASTCLASS
            IF (NOEPOT(CLASS).EQ.NOESYM.OR.
     &           NOEPOT(CLASS).EQ.NOEHDI.OR.
     &           NOEPOT(CLASS).EQ.NOE3DP) THEN
               CALL WRNDIE(-1,'NOEPRI',
     &              'PRINt not possible with POTEntial set to
     $              SYMM, HIGH, or 3DPO')
            ELSE
C     
C     loop over all NOE's belonging to this class
...
C     get energy and other results
                        ...
                        NOEVSM=NOEVSM+ABS(PCDATA(PCDEVI))
                        NOERM2(CLASS)=NOERM2(CLASS)+PCDATA(PCDEVI)**2
                        NOEVS2(CLASS)=NOEVS2(CLASS)+ABS(PCDATA(PCDEVI))
...
C     if deviation greater than threshold then print the NOE
                        IF (QPRINT.AND.(ABS(PCDATA(PCDEVI)).
     $                       GT.THRESH)) THEN
C     
C     check whether we've to print the header for this class
     ...
     &     ' +++++++++++++++++++++++++++++++++++'
                              IF (NOEAVE(NOECND(N)).EQ.NOEM6) THEN
                                 SSAVE='R-6   '
                              ELSE IF (NOEAVE(NOECND(N)).EQ.NOEM3)
     $                                THEN
                                 SSAVE='R-3   '
                              ELSE IF (NOEAVE(NOECND(N)).EQ.NOESUM)
     $                                THEN
                                 SSAVE='sum'
                              ELSE IF (NOEAVE(NOECND(N)).EQ.NOECEN)
     $                                THEN
                                 SSAVE='center'
                              END IF
                              IF (NOEPOT(NOECND(N)).EQ.NOEBIH) THEN
                                 SSPOT='biharmonic '
                              ELSE IF (NOEPOT(NOECND(N)).EQ.NOESQW)
     $                                THEN
                                 SSPOT='square-well'
                              ELSE IF (NOEPOT(NOECND(N)).EQ.NOESOF)
     $                                THEN
                                 SSPOT='soft-square'
                              ELSE IF (NOEPOT(NOECND(N)).EQ.NOELOG)
     $                                THEN
                                 SSPOT='log-normal'
                              END IF
                              WRITE(PUNIT,'(A,F8.3,4A)')
     &                             ' for this class: SCALe=',
     $                             NOESCA(NOECND(N)),
     &                             ' AVERage=',SSAVE,' POTEntial=',
     $                             SSPOT
                              IF (NOEPOT(NOECND(N)).EQ.NOEBIH) THEN
                                 WRITE(PUNIT,'(A,F8.3)')
     &                                '                 TEMPerature=
     ...
     $                                NOEMOF(NOECND(N))
                              ELSE IF (NOEPOT(NOECND(N)).EQ.NOESOF)
     $                                THEN
                                 WRITE(PUNIT,'(A,F8.3,A,I5,A,2F8.3/A,
     $                                I5,A,F8.3,A,F8.3)')
     &                                '                 SQCOnstant=',
     $                                NOECNS(NOECND(N)),
     &                                ' SQEXponent=',NOEEXP(NOECND(N)),
...
C     
     ...
     $                          ' restraint ',NOEPID(N),' =========='
                           DO NORR=NOEORR(N)+1, NOEORR(N+1)
                              ...
                              DO I=NOEIPR(NORR)+1,NOEIPR(NORR+1)
                                 K=NOEILS(I)
                                 WRITE(PUNIT,'(9X,4(1X,A))') SEGID(K),
     $                                RESID(K),RES(K),TYPE(K)
                              END DO
                              WRITE(PUNIT,'(A)') ' set-j-atoms'
     ...
     $                                RESID(K),RES(K),TYPE(K)
                           ...
                           END DO
C     
                           WRITE(PUNIT,'(A,F8.3,A,F5.2,A,F5.2,A,F5.2,
     $                          2A,F8.3,A,F8.3)')
...
C     
     ...
     &              ')=',NOEVIO(I),' of ',NOENU2(I),' NOEs'
      ...
      IF (ITEST.EQ.0) THEN
         ...
         DBPREC =NOEVSM/NOENUML
         CALL DECLAR( 'VAVE', 'DP', ' ', DBCOMP, DBPREC )
         DBPREC=NOENUML
      ...
      ELSE
         ...
         DBPREC =NOEVSM/NOENUML
         CALL DECLAR( 'TEST_VAVE', 'DP', ' ', DBCOMP, DBPREC )
         DBPREC=NOENUML
         CALL DECLAR( 'TEST_NUMBER', 'DP', ' ', DBCOMP, DBPREC )
      END IF
C
      RETURN
      END
...
C local
      INTEGER DDX, DDY, DDZ
C begin
      NDISPR= NORSMX*NOESMX*NOESMX
      DDX=ALLHP(IREAL8(NDISPR))
      DDY=ALLHP(IREAL8(NDISPR))
      DDZ=ALLHP(IREAL8(NDISPR))
      CALL ENOE2(EN,ANALYS,NA,HEAP(DDX),HEAP(DDY),HEAP(DDZ),
     &     HEAP(HPNORR),HEAP(HPNIPR),HEAP(HPNILS),
     &     HEAP(HPNJPR),HEAP(HPNJLS),HEAP(HPNCND),
     &     HEAP(HPNRAV),HEAP(HPNRRV),HEAP(HPNNSP),HEAP(HPNDIS),
     &     HEAP(HPNLOW),HEAP(HPNHIG),HEAP(HPNWGH),HEAP(HPCDIS),
     &     HEAP(HPNCV), HEAP(HPNPID))
      CALL FREHP(DDX,IREAL8(NDISPR))
      CALL FREHP(DDY,IREAL8(NDISPR))
      CALL FREHP(DDZ,IREAL8(NDISPR))
      RETURN
      END
C====================================================================
     ...
     &           NOEDIS,NOELOW,NOEHIG,NOEWGH,NOECDI,NOECV,NOEPID)
C
...
C local
      ...
      DOUBLE PRECISION EL, DEVI, LW, LOGDEVI
...
C
     ...
     &         (NOEPOT(NOECND(N)).EQ.NOE3DP))) THEN
      CALL MULTID(NSTART,NSTOP,N,HN,EN,RN,ANALYS,NA,DDX,DDY,DDZ,NOEORR,
     &           NOEIPR,NOEILS,NOEJPR,NOEJLS,NOECND,NOEDIS,
     &           NOELOW,NOEHIG,NOEWGH,NOECV,VRN)
C
...
C
     ...
     &              (Z(I)-Z(J))*(Z(I)-Z(J))), RSMALL))
       DF=ONE
C
C we have more than one atom and/or more than one possib
      ELSE 
       L1=0
       RN=ZERO
       INORR=0
...
C
        ...
        DF=DF*TWO*C*DEVI
C
       ELSEIF (NOEPOT(NOECND(N)).EQ.NOELOG) THEN
C do lognormal potential
C +++++++++++++++++++++++
C                    2
C E= const log((d/R))  
C
C we calculate the deviation and the deviation of logarithms but keep only
C the deviation of logarithms
C need to limit RN to avoid singularity
        ICLASS=NOECND(N)
        C=MIN(NOEWGH(N)*NOESCA(ICLASS)*NOECNS(ICLASS),NOECEI)
        DEVI=RN-NOEDIS(N)
        LOGDEVI=LOG(MAX(RN,RSMALL)/NOEDIS(N))
        DF=DF*C*TWO*LOGDEVI/RN
        EL  =C*LOGDEVI*LOGDEVI
        DEVI=LOGDEVI
C
...
c         BBB=-NEXP/NSOE*NOEMSW(ICLASS)**(NEXP+NSOE)
c     &      -NOEMAS(ICLASS)/NSOE*NOEMSW(ICLASS)**(NSOE+ONE)
c         EL=C*(AAA+BBB/ABS(DEVI)**NSOE-NOEMAS(ICLASS)*ABS(DEVI))
c         DF=DF*C*(-NSOE*BBB/ABS(DEVI)**(NSOE+1)-NOEMAS(ICLASS))
c     &     *SIGN(1.0D0, DEVI)
...
C
      ...
      END
C======================================================================
      SUBROUTINE MULTID(NSTART,NSTOP,N,HN,EN,RN,ANALYS,NA,DDX,DDY,DDZ,
     &           NOEORR,NOEIPR,NOEILS,NOEJPR,NOEJLS,NOECND,
...
C local
      ...
      DOUBLE PRECISION VRN(20),VDF(20)
...
C calculate FAKT2
      ...
      END IF
C
      ELSE IF (NOELOW(L+N-1).GE.NOELOW(N+N)) THEN
...
C
      ...
      IF (BV.EQ.0) THEN
      FAKT1=SQRT(ELIM1/(2*LIM1*(LIM1**2+2*LIM1*(HDZ-LIM1)+HDZZZ**2+
     &     LIM1**2+LIM1**2+2*LIM1*(HDZ-LIM1)+HDZZZ**2 + LIM1**2)))
      FAKT2=ONE
      ELSE IF (FAKT2.LE.ZERO) THEN
      FAKT2=0.0001D0
      END IF
...
C check whether space was already allocated
      ...
      IF (HPNEXC.NE.0) CALL FREHP(HPNEXC, INTEG4(NOEMAX))
      IF (HPNNUM.NE.0) CALL FREHP(HPNNUM, INTEG4(NOEMAX))
      IF (HPNVIO.NE.0) CALL FREHP(HPNVIO,INTEG4(NOEMAX))
      IF (HPNPID.NE.0) CALL FREHP(HPNPID,INTEG4(NOEMAX))
      IF (HPNSPC.NE.0) CALL FREHP(HPNSPC,IREAL8(NOEMAX))
      IF (HPNPP1.NE.0) CALL FREHP(HPNPP1,IREAL8(NOEMAX))
      IF (HPNPP2.NE.0) CALL FREHP(HPNPP2,IREAL8(NOEMAX))
      IF (HPNHP1.NE.0) CALL FREHP(HPNHP1,IREAL8(NOEMAX))
      IF (HPNHP2.NE.0) CALL FREHP(HPNHP2,IREAL8(NOEMAX))
      IF (HPNVOL.NE.0) CALL FREHP(HPNVOL,IREAL8(NOEMAX))
      IF (HPCVOL.NE.0) CALL FREHP(HPCVOL,IREAL8(NOEMAX))
      IF (HPCDIS.NE.0) CALL FREHP(HPCDIS,IREAL8(NOEMAX))
C
      NOENUM=0
      NOEORN=0
      NOEMAX=0
...
C now allocate new space
      IF (NOENEW.GT.0) THEN
      NOEMAX=NOENEW
      HPCDIS=ALLHP(IREAL8(NOENEW))
      HPCVOL=ALLHP(IREAL8(NOENEW))
      HPNVOL=ALLHP(IREAL8(NOENEW))
      HPNHP2=ALLHP(IREAL8(NOENEW))
      HPNHP1=ALLHP(IREAL8(NOENEW))
      HPNPP2=ALLHP(IREAL8(NOENEW))
      HPNPP1=ALLHP(IREAL8(NOENEW))
      HPNSPC=ALLHP(IREAL8(NOENEW))
      HPNPID=ALLHP(INTEG4(NOENEW))
      HPNVIO=ALLHP(INTEG4(NOENEW))
      HPNCV=ALLHP(INTEG4(NOENEW))
      HPNEXC=ALLHP(INTEG4(NOENEW))
      HPNNUM=ALLHP(INTEG4(NOENEW))
...
C
     ...
     & 'Unable to correctly partition the NOE data within ten trials')
      ENDIF
C
      RETURN
      END
