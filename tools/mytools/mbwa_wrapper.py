...
def __main__():
    ...
    parser.add_option( '', '--tmpdir', dest='tmpdir', help='Use Galaxy tmp directory' )
    ...
    parser.add_option( '-X', '--do_not_build_index', dest='do_not_build_index', action='store_true', help="Don't build index", default=False )
    parser.add_option( '-H', '--suppressHeader', dest='suppressHeader', help='Suppress header' )
    parser.add_option( '-I', '--illumina1.3', dest='illumina13qual',  action='store_true', help='Input FASTQ files have Illumina 1.3 quality scores' )
    ...
    tmp_index_dir = tempfile.mkdtemp(dir=options.tmpdir)
    tmp_dir = tempfile.mkdtemp(dir=options.tmpdir)
    # index if necessary
    if options.fileSource == 'history' and not options.do_not_build_index:
        ...
        cmd1 = 'mbwa index %s %s' % ( indexing_cmds, ref_file_name )
        try:
    ...
    cmd2 = 'mbwa aln -f %s %s %s %s' % ( tmp_align_out_name, aligning_cmds, ref_file_name, fastq )
    cmd2b = ''
    if options.genAlignType == 'paired':
        cmd2b = 'mbwa aln -f %s %s %s %s' % ( tmp_align_out2_name, aligning_cmds, ref_file_name, rfastq )
        cmd3 = 'mbwa sampe -P %s %s %s %s %s %s >> %s' % ( gen_alignment_cmds, ref_file_name, tmp_align_out_name, tmp_align_out2_name, fastq, rfastq, options.output )
    else:
        cmd3 = 'mbwa samse %s %s %s %s >> %s' % ( gen_alignment_cmds, ref_file_name, tmp_align_out_name, fastq, options.output )
    # perform alignments
    buffsize = 1048576
    try:
