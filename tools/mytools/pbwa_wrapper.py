...
def __main__():
    ...
    parser.add_option( '', '--parallel', dest='parallel', help='Use parallel bwa or bwa' )
    ...
    if options.parallel == 'yes':
        cmd2 = 'pbwa aln -f %s %s %s %s' % ( tmp_align_out_name, aligning_cmds, ref_file_name, fastq )
    else:
        cmd2 = 'bwa aln %s %s %s > %s' % ( aligning_cmds, ref_file_name, fastq, tmp_align_out_name )
    cmd2b = ''
    if options.genAlignType == 'paired':
        if options.parallel == 'yes':
            cmd2b = 'pbwa aln -f %s %s %s %s' % ( tmp_align_out2_name, aligning_cmds, ref_file_name, rfastq )
        else:
            cmd2b = 'bwa aln %s %s %s > %s' % ( aligning_cmds, ref_file_name, rfastq, tmp_align_out2_name )
        cmd3 = 'bwa sampe -P %s %s %s %s %s %s >> %s' % ( gen_alignment_cmds, ref_file_name, tmp_align_out_name, tmp_align_out2_name, fastq, rfastq, options.output )
    else:
