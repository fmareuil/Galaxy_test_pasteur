...
# Two files are generated for option '-b': one for the strand plus and one for the strand minus
    ...
    if options.outname[-8:] != '.txt.bz2':
        outname_plus = options.outname + '_plus.txt.bz2'
        outname_minus = options.outname + '_minus.txt.bz2'
    else:
        outname_plus = options.outname[:-8] + '_plus.txt.bz2'
        outname_minus = options.outname[:-8] + '_minus.txt.bz2'
...
if len(output) == 1:
    ...
    compressResults(output[0], outname_plus)
    compressResults(output[1], outname_minus)
    print('New files created:\n\t%s for strand +\n\t%s for strand -\n' % (outname_plus, outname_minus))
