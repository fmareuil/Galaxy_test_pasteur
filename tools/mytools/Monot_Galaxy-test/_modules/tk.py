...
\n(iv) Provide a directory for the output file;\n(v) Set eventually optional parameters.'
    ...
    def launchAnalysis(self):
        ...
        if inputs[11] == 'no':
            ...
            outname = ['plus', 'minus']
            outname[0] = dir + 'export_map2cov_plus.txt.bz2'
            outname[1] = dir + 'export_map2cov_minus.txt.bz2'
            # Output files test
            if os.path.isfile(outname[0]) or os.path.isfile(outname[1]):
                i = 1
                basename_0 = outname[0][:-8]
                basename_1 = outname[1][:-8]
                outname[0] = '%s(%i).txt.bz2' % (basename_0, i)
                outname[1] = '%s(%i).txt.bz2' % (basename_1, i)
                while os.path.isfile(outname[0]) or os.path.isfile(outname[1]):
                    i += 1
                    outname[0] = '%s(%i).txt.bz2' % (basename_0, i)
                    outname[1] = '%s(%i).txt.bz2' % (basename_1, i)
    ...
    def compute(self, inputs):
        ...
        if len(output) == 1:
            ...
            compressResults(output[0], outname[0])
            compressResults(output[1], outname[1])
            self.display(win, 'Path to the output file: Strand plus', 'label')
            self.display(win, outname[0], 'text')
            self.display(win, 'Path to the output file: Strand minus', 'label')
            self.display(win, outname[1], 'text')
            self.display(win, '\nData analysis: DONE', 'label')
