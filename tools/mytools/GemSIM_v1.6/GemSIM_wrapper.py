...
import os
import zlib
from os.path import basename, abspath, realpath, dirname, join as joinpath
...
def safemembers(members):
    ...
    for finfo in members:
        if finfo.isfile():
            finfo.name = basename(finfo.name)
...
def __main__():
    ...
    if options.r:
        command["-r"] = options.r
    if options.R:
        tarmembRs = tarfile.open(options.R,"r:")
        tarmembRs.extractall(".", members=safemembers(tarmembRs))
	tarmembRs.close()
	#if not dirname(options.R):
        command["-R"] = "."
	#else:
	#    command["-R"] = dirname(options.R)
    if options.a:
        command["-a"] = options.a    
    if options.n:
        command["-n"] = options.n   
    if options.g:
        command["-g"] = options.g	
    if options.G:
        tarmembGs = tarfile.open(options.G,"r:")
        tarmembGs.extractall(path=".", members=safemembers(tarmembGs))
	tarmembGs.close()
	#if not dirname(options.G):
        command["-G"] = "."
	#else:
        #    command["-G"] = dirname(options.G)	
    if options.l:
        command["-l"] = options.l		
    if options.m:
        command["-m"] = options.m
    ...
    try:
        print cmd
        proc = subprocess.Popen( args=cmd, shell=True, stderr=subprocess.STDOUT)
        proc.wait()
    except Exception, e:
        print >> sys.stderr, 'Error running %s, %s ' % (cmd, str(e) )
        sys.exit( 1 )
