...
## PATCH GALAXY MAREUIL
def check_type(filein):
    import subprocess as sub
    import string
    p = sub.Popen('file %s' % filein, shell=True, stdout=sub.PIPE, stderr=sub.PIPE)
...
def parseModel(gzipFile,paired,readlen):
    ...
    try: 
        type_file = check_type(gzipFile)
	if type_file == "gzip":
            file=gzip.open(gzipFile,'rb')
	else:
	    file=open(gzipFile,'r')
## PATCH GALAXY MAREUIL
