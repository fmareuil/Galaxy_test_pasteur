...
if [ -n "$GALAXY_RUN_ALL" ]; then
    ...
    echo "Handling api with log file api.log..."
    python ./scripts/paster.py serve universe_wsgi.ini --server-name=api --pid-file=api.pid --log-file=api.log $@
else
    python ./scripts/paster.py serve universe_wsgi.ini --server-name=api --pid-file=api.pid $@
fi
