...
echo $GALAXY_UNIVERSE_CONFIG_DIR
...
if [ -n "$GALAXY_RUN_ALL" ]; then
    servers=`sed -n 's/^\[server:\(.*\)\]/\1/  p' universe_wsgiapi.ini | xargs echo`
    ...
    for server in $servers; do
        ...
        python ./scripts/paster.py serve universe_wsgiapi.ini --server-name=$server --pid-file=$server.pid --log-file=$server.log $@
    done
else
    python ./scripts/paster.py serve universe_wsgiapi.ini $@
fi
